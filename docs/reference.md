...
| Mesh repair | Implemented for core cleanup | `repair` report step; mesh metadata records before/after duplicate polygon, degenerate triangle, boundary edge, boundary gap, non-manifold edge, T-junction, and non-orientable shared-edge counts | Add T-junction sewing, boundary-gap stitching, non-manifold cracking, flipped-component metrics, and configurable orientation strategies |
